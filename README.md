...
Install into the Hermes gateway's own venv, the same one that runs `hermes_cli.main gateway run`. Not `pipx`: a pipx install is isolated, so the gateway cannot import the adapter.
...
Hermes creates its venv with `uv`, so that venv has no `pip` binary and no `pip` module. Use `uv pip install --python` as shown, not `<hermes-venv>/bin/pip`.
...
Re-running the `uv pip install` upgrades the adapter without re-running the installer. `stagewhisper-hermes-install --uninstall` removes the shim.
