...
@pytest.mark.asyncio
async def test_a_send_after_the_turn_closed_is_delivered_as_a_new_message() -> None:
    ...
    try:
        ...
        try:
            ...
            assert capture.received[-1]["status"] == "completed"
            ...
            assert capture.received[-1]["reply_text"] == "stray fallback send"
            assert capture.received[-1]["status"] == "message"
            assert capture.received[-1]["task_id"] != handle.task_id
...
@pytest.mark.asyncio
async def test_a_stale_dispatch_context_send_never_reaches_a_newer_turn() -> None:
    from hermes_stagewhisper_plugin.models import ValidatedEvent
    ...
    try:
        ...
        try:
            ...
            a_messages = [b for b in capture_a.received if b["status"] == "message"]
            assert len(a_messages) == 1
            assert a_messages[0]["task_id"] != handle_a.task_id
            assert [b for b in capture_b.received if b["status"] == "message"] == []
        finally:
            await adapter._client.close()
    finally:
...
@pytest.mark.asyncio
async def test_a_silent_chat_turn_does_not_invent_a_terminal_error() -> None:
    from hermes_stagewhisper_plugin.models import ValidatedEvent
    ...
    try:
        ...
        try:
            ...
            assert not any(body["status"] == "errored" for body in capture.received)
...
@pytest.mark.asyncio
async def test_a_send_after_a_timed_out_turn_arrives_as_a_new_message(monkeypatch) -> None:
    from hermes_stagewhisper_plugin.models import ValidatedEvent
    import hermes_stagewhisper_plugin.adapter as adapter_mod
    ...
    try:
        ...
        try:
            ...
            messages = [b for b in capture.received if b["status"] == "message"]
            assert len(messages) == 1
            assert messages[0]["reply_text"] == "late reply"
            assert messages[0]["task_id"] != handle.task_id
            assert messages[0]["user_message_id"] == "umid-late"
        finally:
            await adapter._client.close()
    finally:
        await capture.stop()
...
async def test_get_chat_info_returns_dm_dict() -> None:
    adapter = _make_adapter()
    info = await adapter.get_chat_info("sw:abc-123:chat")
    assert info == {"name": "sw:abc-123:chat", "type": "dm"}


async def test_connect_accepts_is_reconnect_and_rebinds_the_listener() -> None:
    adapter = _make_adapter()
    health_url = f"http://127.0.0.1:{adapter._port}/v1/health"
    try:
        assert await adapter.connect() is True
        async with ClientSession() as client, client.get(health_url) as response:
...
@pytest.mark.asyncio
async def test_a_reply_produced_just_after_the_agent_returns_still_lands_in_the_same_turn(
    monkeypatch,
) -> None:
    import hermes_stagewhisper_plugin.adapter as adapter_mod
    from hermes_stagewhisper_plugin.models import ValidatedEvent

    monkeypatch.setattr(adapter_mod, "TURN_IDLE_GRACE_S", 3.0)
    ...
    try:
        ...
        await _open_client(adapter)
        try:
            chat_id = "sw:session-deferred:chat"
            background: dict[str, asyncio.Task] = {}

            async def handler(event: Any) -> None:
                async def deferred() -> None:
                    await asyncio.sleep(0.5)
                    await adapter.send(event.source.chat_id, "an LSTM is a recurrent net")
            ...
            event = ValidatedEvent(
                ...
                text="what's LSTM?",
                is_final=True,
                user_message_id="umid-deferred",
                parent_message_id=None,
                callback_url=f"http://127.0.0.1:{capture.port}",
                callback_token="callback-token-deferred-aaaa",
                raw={},
            )
            handle = CallbackHandle(
                task_id=event.task_id,
            ...
            messages = [b for b in capture.received if b["status"] == "message"]
            assert len(messages) == 1
            assert messages[0]["reply_text"] == "an LSTM is a recurrent net"
            assert messages[0]["task_id"] == handle.task_id

            terminals = [
                b
                for b in capture.received
                if b["status"] in ("completed", "errored", "silent")
            ]
...
@pytest.mark.asyncio
async def test_an_agent_still_typing_keeps_its_turn_open_until_the_answer_arrives(
    monkeypatch,
) -> None:
    import hermes_stagewhisper_plugin.adapter as adapter_mod
    from hermes_stagewhisper_plugin.models import ValidatedEvent

    monkeypatch.setattr(adapter_mod, "TURN_IDLE_GRACE_S", 0.4)
    ...
    try:
        ...
        await _open_client(adapter)
        try:
            chat_id = "sw:session-typing-alive:chat"
            background: dict[str, asyncio.Task] = {}
            ...
            event = ValidatedEvent(
                task_id="task-typing-alive",
                session_id="session-typing-alive",
                reason="chat_message",
                occurred_at="2026-01-01T00:00:00Z",
                text="what's LSTM?",
                is_final=True,
                user_message_id="umid-typing-alive",
                parent_message_id=None,
                callback_url=f"http://127.0.0.1:{capture.port}",
                callback_token="callback-token-typingalive-a",
                raw={},
            )
            handle = CallbackHandle(
                task_id=event.task_id,
...
@pytest.mark.asyncio
async def test_typing_after_a_first_part_keeps_the_turn_open_for_the_final_part(
    monkeypatch,
) -> None:
    import hermes_stagewhisper_plugin.adapter as adapter_mod
    from hermes_stagewhisper_plugin.models import ValidatedEvent

    monkeypatch.setattr(adapter_mod, "TURN_QUIET_PERIOD_S", 0.4)
    ...
    try:
        ...
        await _open_client(adapter)
        try:
            chat_id = "sw:session-two-parts:chat"
            background: dict[str, asyncio.Task] = {}

            async def handler(event: Any) -> None:
                await adapter.send(event.source.chat_id, "let me check")
            ...
            event = ValidatedEvent(
                task_id="task-two-parts",
                session_id="session-two-parts",
                reason="chat_message",
                occurred_at="2026-01-01T00:00:00Z",
                text="explain",
                is_final=True,
                user_message_id="umid-two-parts",
                parent_message_id=None,
                callback_url=f"http://127.0.0.1:{capture.port}",
                callback_token="callback-token-twoparts-aaa",
                raw={},
            )
            handle = CallbackHandle(
                task_id=event.task_id,
            ...
            messages = [b for b in capture.received if b["status"] == "message"]
            assert [b["reply_text"] for b in messages] == [
                "let me check",
                "here is the answer",
            ]
            assert {b["task_id"] for b in messages} == {handle.task_id}
